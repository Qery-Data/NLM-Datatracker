...
#Gjennomsnitt og median lønn måned yrker nivå 4
ssburl = 'https://data.ssb.no/api/v0/no/table/11418/'
query = {
  "query": [
    {
...
df_new.dropna(inplace=True)
df_new['Forskjell i kr']=df_new['Gjennomsnitt']-df_new['Median']
df_new['Forskjell i pst']=(df_new['Gjennomsnitt']-df_new['Median'])/(df_new['Gjennomsnitt'])*100
df_new.to_csv('data/SSB_lonn_yrke_gjennomsnitt_median.csv', index=True)
json_object = json.loads(result.text)
raw_date = json_object["updated"]
parsed_date = datetime.strptime(raw_date, '%Y-%m-%dT%H:%M:%SZ')
chart_date = 'Data sist publisert: ' + parsed_date.strftime ('%d/%m/%y')
...
date_string = 'Kvinners månedslønn som andel av menns lønn etter ulike beregningsmåter.'+' Tall for ' + title_date +'.'
#Update DW 1
chartid = 'Fn94r'
url = "https://api.datawrapper.de/v3/charts/" + chartid + '/'
payload = {"metadata": {"annotate": {"notes": chart_date}}}
